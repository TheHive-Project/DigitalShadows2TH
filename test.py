...
def run():
    ...
    # since1 = (datetime.datetime.now() - datetime.timedelta(minutes=60)).isoformat() + 'Z'
    # DigitalShadowsApi.find_incident(dsapi, since1)
    # since2 = "{}/{}".format((datetime.datetime.now() - datetime.timedelta(minutes=600)).isoformat(),
    #                         datetime.datetime.now().isoformat())
    # DigitalShadowsApi.find_intel_incident(dsapi, since2)
    DigitalShadowsApi.get_intel_incident_iocs(dsapi, 1844578)
    ...
    #         create_th_alert(thapi, response.json())
    #     else:
